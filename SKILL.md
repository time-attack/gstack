...
- User asks to merge an existing PR, "land the pr", "merge it", "get it merged" (merge only, no deploy) → invoke `/land`
...
- User asks to set up search, install web search MCP, connect the Exa Search MCP, or make Search Before Building work → invoke `/setup-search-mcp`
