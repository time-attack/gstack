...
preamble-tier: 3
version: 1.0.0
description: Auto-review pipeline — reads the full CEO, PM, design, eng, and DX review skills from disk and runs them sequentially with auto-decisions using 6 decision principles. (gstack)
...
/autoplan reads the full CEO, PM, design, eng, and DX review skill files from disk and follows
...
- `~/.claude/skills/gstack/plan-pm-review/SKILL.md` (only when Phase 1.5's run condition is met — that depends on the CEO review output, so load it lazily at the start of Phase 1.5; the section skip list below applies to it the same as any other loaded skill file)
...
Read `~/.claude/skills/gstack/plan-pm-review/SKILL.md` now if it was not
already loaded in Step 3 (the Step 3 section skip list applies to it too).
...
if command -v jq >/dev/null 2>&1; then
  # Collect entries from all phases, scoped to current branch + commit window.
  ...
  for phase in ceo-review pm-review design-review eng-review devex-review; do
  ...
  AGGREGATED_TASKS=$(jq -s \
    'group_by([.component, (.files | sort), .title])
     | map(
         ...
         sort_by({P1:0,P2:1,P3:2}[.priority] // 99, {"ceo-review":0,"pm-review":1,"design-review":2,"eng-review":3,"devex-review":4}[.phase] // 99) | .[0]
       )
     | sort_by({P1:0,P2:1,P3:2}[.priority] // 99, {"ceo-review":0,"pm-review":1,"design-review":2,"eng-review":3,"devex-review":4}[.phase] // 99)
