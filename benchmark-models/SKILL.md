...
GPT (via Codex CLI), Gemini, Grok, and Ollama (local) side-by-side — compares
...
```bash
"$BIN" --prompt "unused, dry-run" --models claude,gpt,gemini,grok,ollama --dry-run
```
...
If ALL show NOT READY: stop with a clear message — benchmark can't run without at least one authed provider. Suggest `claude login`, `codex login`, `gemini login` / `export GOOGLE_API_KEY`, `grok` login / `export XAI_API_KEY`, or for local: install Ollama from https://ollama.com then `ollama pull qwen2.5-coder:7b && ollama serve`.
