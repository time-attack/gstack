...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"community-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"community-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"community-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
🟡 FIX THEN MERGE (high impact, minor issues)
  #789 — "Add /delegate skill" by @user (Impact: 8, Effort: 5, Score: 48)
    ...
    → Fix needed: convert to .tmpl, use ## Preamble (run first)
...
cd /path/to/gstack && claude --print -p "/community-review --issue"
```

This creates a GitHub Issue digest, so Monday morning the maintainer opens one
issue and sees everything prioritized.

Never include `--post` in unattended runs. Auto-commenting on contributors' PRs
is an explicit per-run opt-in the maintainer chooses interactively — a bot that
comments while nobody is watching is how contributors get burned.
