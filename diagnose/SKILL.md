...
preamble-tier: 2
version: 1.0.0
description: Deep diagnostic root cause analysis — overcomes the model's natural bias towards action, forcing evidence-based investigation before any conclusion. (gstack)
allowed-tools:
  - Bash
  - Read
  - Grep
...
failures, or multi-system problems.
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"diagnose","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"diagnose","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/gstack/bin/gstack-learnings-log '{"skill":"diagnose","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}' --signal SIGNAL
```
