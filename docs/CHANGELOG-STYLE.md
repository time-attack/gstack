...
Contributor guidance for the gstack repo. Read it when writing a CHANGELOG
entry or picking a version bump for THIS repo. It moved out of CLAUDE.md so
it costs nothing on the sessions that never ship anything.

It stays in `docs/` rather than `skills/ship/`: `skills/` ships to every user
via `npx skills add`, and these are gstack's own release conventions in
Garry's voice, not rules to impose on someone else's project.
