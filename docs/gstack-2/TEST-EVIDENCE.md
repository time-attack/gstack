...
| Harness-4 host adapter offline suite (`bun test test/gstack2-host-adversarial.test.ts`) | **32 pass / 0 fail**, 183 assertions. | Covers the per-host adapter layer (codex byte-identical; claude, cursor, pi event mapping, slash-invocation prompt transform, consolidated-final-message parsing, plan-tool classification) plus all retained-run pins. Deterministic; not live-host proof. |
