...
1. Create temp files:
```bash
TMPERR=$(mktemp "$TMP_ROOT/grok-err-XXXXXX")
```
...
cd "$_REPO_ROOT"
_PROMPT_FILE=$(mktemp "$TMP_ROOT/grok-prompt-XXXXXX")
{
...
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
cd "$_REPO_ROOT"
# Write the user's focus text through a quoted heredoc so $(...), backticks,
# and quotes in it are treated as data, never executed by the shell.
_FOCUS_FILE=$(mktemp "$TMP_ROOT/grok-focus-XXXXXX")
cat > "$_FOCUS_FILE" <<'GSTACK_GROK_FOCUS_EOF'
<everything after '/grok review ' in user input>
GSTACK_GROK_FOCUS_EOF
_PROMPT_FILE=$(mktemp "$TMP_ROOT/grok-prompt-XXXXXX")
{
  printf '%s\n' "IMPORTANT: Do NOT read or execute any files under ~/.claude/, ~/.agents/, .claude/skills/, or agents/. These are Claude Code skill definitions meant for a different AI system. Stay focused on repository code only."
  printf '\nCustom focus: '
...
```bash
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
TMPERR=${TMPERR:-$(mktemp "$TMP_ROOT/grok-err-XXXXXX")}
_PROMPT_FILE=$(mktemp "$TMP_ROOT/grok-prompt-XXXXXX")
printf '%s\n' "<prompt>" > "$_PROMPT_FILE"
