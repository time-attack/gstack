#!/usr/bin/env bash
# Migration: v1.61.0.0 — heal the v1.27 rename for already-migrated installs
#
...
# lands as something other than v1.61.0.0, rename this file and its test.
#
...
DONE="${MIGRATION_DIR}/v1.61.0.0.done"
...
if ! command -v gh >/dev/null 2>&1; then
  echo "  [v1.61.0.0] gh CLI not available — cannot verify the artifacts repo rename." >&2
...
if ! gh auth status >/dev/null 2>&1; then
  echo "  [v1.61.0.0] gh is not authenticated — skipping the artifacts rename check." >&2
...
if ! gh repo view "$OWNER/$OLD_NAME" >/dev/null 2>&1; then
  echo "  [v1.61.0.0] neither $OWNER/$NEW_NAME nor $OWNER/$OLD_NAME exists on GitHub — nothing to heal." >&2
  finish
fi

echo "  [v1.61.0.0] healing the v1.27 artifacts rename: $OWNER/$OLD_NAME → $NEW_NAME" >&2
if gh repo rename "$NEW_NAME" --repo "$OWNER/$OLD_NAME" --yes >/dev/null 2>&1; then
  echo "  [v1.61.0.0] renamed — artifacts sync to $REMOTE_URL works again." >&2
  finish
fi

echo "  [v1.61.0.0] WARNING: rename failed (permissions or org policy?). Run manually:" >&2
echo "    gh repo rename $NEW_NAME --repo $OWNER/$OLD_NAME --yes" >&2
