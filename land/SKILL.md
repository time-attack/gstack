...
- `/land --override-review` — bypass the GitHub PR review-approval hard gate (Step 3.5a-ter) when a requested human reviewer hasn't approved. This is the ONLY way past that gate — `--fast` never bypasses it, and neither does the "merge anyway" option in the readiness prompt. Use it only when you ARE the reviewer and you approve this change.
...
2. Parse arguments. If the user passed `#NNN`, use that PR number. If they passed `--fast`, remember that for Step 3.5. From here on, `<NNN>` in every `gh pr view` / `gh pr checks` command below means this PR's number (from the argument, or from the detection in the next step) — always pass it explicitly, so `/land #123` never reads the current branch's PR by accident.
...
```bash
gh pr checks <NNN> --json name,state,bucket
```
...
```bash
gh pr view <NNN> --json mergeable -q .mergeable
```
...
```bash
gh pr checks <NNN> --watch --fail-fast
```
...
Read the PR's own VERSION — not the current checkout's, which may be a different branch when landing by PR number:

```bash
HEAD_BRANCH=$(gh pr view <NNN> --json headRefName -q .headRefName 2>/dev/null)
git fetch origin "$HEAD_BRANCH" 2>/dev/null
BRANCH_VERSION=$(git show "origin/$HEAD_BRANCH:VERSION" 2>/dev/null | tr -d '\r\n[:space:]' || echo "")
BASE_BRANCH=$(gh pr view <NNN> --json baseRefName -q .baseRefName 2>/dev/null || echo main)
...
```bash
gh pr view <NNN> --json reviewDecision,reviewRequests,latestReviews
```
...
**How to fetch collaborators for the "solo repo" check** (self-contained — run as one block):
...
echo "OTHER_COLLABS=$_OTHER_COLLABS"
```

If the count is greater than 0, treat missing reviews as a warning/blocker per the table above. If it is 0, treat as solo repo and pass. The collaborators API needs push access — if the call fails (403, network), the count falls back to 0 and the gate passes as solo. Fail-open is deliberate: this is a discipline gate, not a security boundary, and blocking every merge on a flaky API call would get the whole gate disabled.

**If BLOCKER:** Include the review state in the readiness report under the `GITHUB REVIEW` line (see 3.5e) and recommend option B (hold off). Do NOT let the user override with option C for this specific blocker — review bypass requires a different, explicit override. `--fast` does not apply either (it never skips a blocker).

**Explicit override for the GitHub review blocker:** Only if the user invokes `/land --override-review` (or answers an explicit `AskUserQuestion` with a dedicated "I am the reviewer and I approve this" option) can this blocker be bypassed. Default is refuse.

**Why this matters:** Free GitHub plans on private repos have no branch protection, so discipline is the only gate. Without this check, `/land` would happily merge PRs that the assigned reviewer never saw. That is the exact failure mode this sub-step exists to prevent.

### 3.5b: Test results

**Free tests — run them now.** Read CLAUDE.md for the project's test command. If CLAUDE.md doesn't specify one, ask via AskUserQuestion (let the user give the command, or confirm the project has no test suite), then persist the answer to CLAUDE.md so we never ask again. Run the test command, appending `2>&1 | tail -10` to capture the exit code and the last lines of output.

If the project genuinely has no test suite: **WARNING** (note it in the readiness report), not a blocker.
...
```bash
gh pr view <NNN> --json body -q .body
git log --oneline $(gh pr view <NNN> --json baseRefName -q .baseRefName 2>/dev/null || echo main)..HEAD | head -20
```
...
```bash
git diff --name-only $(gh pr view <NNN> --json baseRefName -q .baseRefName 2>/dev/null || echo main)...HEAD -- README.md CHANGELOG.md ARCHITECTURE.md CONTRIBUTING.md CLAUDE.md VERSION
```
...
║  └─ GITHUB REVIEW: APPROVED / BLOCKED ({why}) / solo     ║
║                                                          ║
║  TESTS                                                   ║
...
- If there are any **blockers** (failing free tests, the GitHub review gate from 3.5a-ter): `--fast` does NOT apply — list the blockers and recommend B below. Never auto-proceed past a blocker.
...
**If the blocker is "GitHub PR review not approved" (3.5a-ter), option C is NOT available.** The user must either get the approval on GitHub or explicitly re-run with `/land --override-review`. Do not present option C for this blocker.

If B: **STOP** with specific next steps (run `/review`, run E2E, run `/document-release`, get the GitHub approval, or fix the PR body).
...
`trunk`) and continue. **If B:** write `Merge queue: none` into a `## Merge Configuration`
section of CLAUDE.md (create the section if absent), then continue with the `none` path.
Either way, the written `## Merge Configuration` settles it — later runs read the config
and never re-ask.
...
### 4.1c: Resolve the merge method (`none` regime only)

For a queue regime (`github` / `trunk`), the queue owns the merge method — skip this
sub-step entirely. For the `none` regime, resolve the merge method once, remember it,
and substitute it for `<method>` in the 4.2 merge command. Three-tier priority:

1. **CLAUDE.md Deploy Configuration** — read CLAUDE.md's `## Deploy Configuration`
   section for a `- Merge method:` line. Ignore it when the line is missing, still
   holds the literal template placeholder (`{squash/merge/rebase}`), or isn't exactly
...
Tell the user the result: "Merge method: `<method>` (source: {CLAUDE.md/repo/default})."
...
- **none** → `gh pr merge <pr> --squash --delete-branch`. The helper always squashes, so use it only when the method resolved in 4.1c is `squash`. If 4.1c resolved `merge` or `rebase`, skip the helper and merge directly with the resolved method substituted for `<method>`:

  ```bash
  gh pr merge <NNN> --<method> --delete-branch
  ```

  The 4.2a post-failure check and the rest of the `none` path apply identically either way.
...
```bash
gh pr view <NNN> --json state,mergeCommit,mergedAt,mergedBy
```
...
```bash
gh pr view <NNN> --json mergeCommit -q .mergeCommit.oid
```
...
```bash
gh pr view <NNN> --json autoMergeRequest -q .autoMergeRequest
```
...
- **exit 0 with `ENQUEUE_HOW=already merged`** — the PR didn't queue; it MERGED before the poll (e.g. auto-merge fired instantly, or the github regime fell back to a direct squash). Do NOT print the enqueue summary — go to Step 5 (write-state confirms the SHA and writes `last-land.json`), then Step 6's **land summary**.
- **any other exit 0 / `ENQUEUED=...`** — the PR is in the queue and will land on `<base>` on its own. Do NOT run Step 5 (there is no merge SHA yet — `confirm-enqueue` wrote a lightweight `last-enqueue.json`). Go to Step 6's **enqueue summary**, and surface the `WATCH_CHECK` / `WATCH_DASHBOARD` lines from the output so the user knows where to look.
