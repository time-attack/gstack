...
**Free tests — run them now.** Read CLAUDE.md for the project's test command. If CLAUDE.md doesn't specify one, ask via AskUserQuestion (let the user give the command, or confirm the project has no test suite), then persist the answer to CLAUDE.md so we never ask again. Run it, capture exit code and output.

```bash
<test command from CLAUDE.md> 2>&1 | tail -10
```
