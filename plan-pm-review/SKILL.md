...
interactive: true
version: 1.0.0
description: Product manager-mode plan review. (gstack)
...
Prioritize with frameworks (RICE/ICE/WSJF),
...
what to cut, what to defer, or who the feature is actually for.

Voice triggers (speech-to-text aliases): "PM review", "product review", "prioritize the plan", "RICE scoring", "acceptance criteria", "roadmap ranking".
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-pm-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"plan-pm-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"plan-pm-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
> "No design doc found for this branch. `/office-hours` or `/plan-ceo-review` produces a structured problem
...
mkdir -p "$TASKS_DIR"
TASKS_FILE="$TASKS_DIR/tasks-pm-review-$(date +%Y%m%d-%H%M%S).jsonl"
...
```bash
~/.claude/skills/gstack/bin/gstack-learnings-log '{"skill":"plan-pm-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}' --signal SIGNAL
```
