...
preamble-tier: 1
version: 1.0.0
description: Check progress of a gstack plan against the current codebase and git log. (gstack)
triggers:
  - plan status
  - how far along
...
"what's left", "check plan progress", "where are we on the plan".
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-status","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
_MB=$(git merge-base HEAD "${_BASE:-main}" 2>/dev/null || git merge-base HEAD master 2>/dev/null)
_GIT_LOG=$(git log --oneline "${_MB:+$_MB..}HEAD" 2>/dev/null | head -30)
echo "BRANCH_COMMITS:"
echo "$_GIT_LOG"
...
**Git evidence:**
```bash
# Detect the base branch, then list commits + files changed since diverging from it
_BASE=$(git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null | sed 's|refs/remotes/origin/||')
_MB=$(git merge-base HEAD "${_BASE:-main}" 2>/dev/null || git merge-base HEAD master 2>/dev/null)
git log --oneline "${_MB:+$_MB..}HEAD" 2>/dev/null
git diff --name-only "${_MB:+$_MB..}HEAD" 2>/dev/null
```
