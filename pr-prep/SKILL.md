...
2. Determine the base branch: use the base branch detected in Step 0.
   Honor `--base <name>` flag override. Substitute it wherever a code
   block below says `<base>` (each bash block runs in a fresh shell —
   no variables persist between blocks).
...
4. **Read the upstream `CONTRIBUTING.md` (if present)** and surface its
   ...
   Substitute `<repo>` with the upstream repo resolved in item 3:
   ...
   - Branch naming / commit message conventions. Look for "branch
     ...
     `git log upstream/<base> --no-merges -20 --format='%s%n%b%n--'`.
...
5. Sanity-check: at least 1 commit in `<base>..HEAD`. If zero,
   abort with "no commits to audit; you're already on the base branch".

```bash
BASE="<base>"  # the base branch detected in Step 0 (or --base override)
...
For each commit's query, run the block below, substituting `<repo>` with
the upstream repo resolved in Step 1 and `<query>` with this commit's
search query from Step 2 (fresh shell per block — no variables carry over):
...
# Closed in last 90 days (might be an unreleased fix on the base branch)
...
re-deriving the thresholds inline. Build the candidate JSON for the
commit, write it inline in the same block (no cross-block variables):

```bash
echo '<candidate JSON for this commit>' | ~/.claude/skills/gstack/bin/gstack-pr-prep-score
...
Run the block below once per CLEAN-bucketed commit from Step 4,
substituting `<sha>` each time (fresh shell per block — no list
variable carries over):

```bash
if command -v codex >/dev/null 2>&1; then
  subject=$(git log -1 --format=%s <sha>)
  codex review "Review commit <sha> '${subject}' for correctness,
...
| Flag | Default | Effect |
|---|---|---|
| `--base <name>` | detected base branch (Step 0) | Base branch for commit walk |
