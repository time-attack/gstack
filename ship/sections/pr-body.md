...
gh pr create --base <base> ${_REVIEWERS:+--reviewer "$_REVIEWERS"} --title "v$NEW_VERSION <type>: <summary>" --body-file "$PR_BODY_FILE"
rm -f "$PR_BODY_FILE"
```
...
# (See Step 19 idempotency block + bin/gstack-pr-title-rewrite.sh for the rule.)
_REVIEWERS="<resolved reviewers from above, comma-separated, or empty>"
glab mr create -b <base> ${_REVIEWERS:+--reviewer "$_REVIEWERS"} -t "v$NEW_VERSION <type>: <summary>" -d "$(cat <<'EOF'
<MR body from above>
EOF
)"
```
