...
```bash
$GSTACK_BIN/gstack-learnings-log '{"skill":"learn","type":"TYPE","key":"KEY","insight":"INSIGHT","confidence":N,"source":"user-stated","files":["FILE1"]}' 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-learnings-log skipped"
```

## Context Recovery

At session start or after compaction, recover recent project context. (Migrated from the 1.x session-intelligence preamble, which the 1.x tree deletion dropped without a surviving carrier; the runtime scripts it reads still ship.)
