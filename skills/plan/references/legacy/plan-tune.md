...
4. If A or B: enable:
   ```bash
   $GSTACK_BIN/gstack-config set question_tuning true 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-config skipped"
   ```
...
1. Ask FIVE one-per-dimension declaration questions via individual
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   $GSTACK_BIN/gstack-developer-profile --read >/dev/null 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-developer-profile skipped"
...
```bash
$GSTACK_BIN/gstack-developer-profile --profile 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-developer-profile skipped"
```
...
4. Write:
   ```bash
   $GSTACK_BIN/gstack-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}' 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-question-preference skipped"
   ```
...
```bash
$GSTACK_BIN/gstack-developer-profile --gap 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-developer-profile skipped"
```
...
```bash
$GSTACK_BIN/gstack-question-preference --stats 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-question-preference skipped"
eval "$($GSTACK_BIN/gstack-slug 2>/dev/null)"
eval "$($GSTACK_BIN/gstack-paths 2>/dev/null)"
_LOG="$GSTACK_STATE_ROOT/projects/${PROJECT_ID:-unknown}/question-log.jsonl"
if [ -f "$_LOG" ]; then
...
$GSTACK_BIN/gstack-developer-profile --profile 2>/dev/null | bun -e "
...
" 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-developer-profile skipped"
echo '---DISTILL---'
$GSTACK_BIN/gstack-distill-free-text --status 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-distill-free-text skipped"
```
...
1. Show the proposals:
   ```bash
   $GSTACK_BIN/gstack-distill-apply --list 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-distill-apply skipped"
   ```
...
3. **On accept** (Y): apply via the bin. The skill also publishes the
   ...
   $GSTACK_BIN/gstack-distill-apply --proposal N --gbrain-published true|false 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-distill-apply skipped"
   ```

   For `preference`:
   ```bash
   $GSTACK_BIN/gstack-distill-apply --proposal N 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-distill-apply skipped"
   ```
...
1. Run distill:
   ```bash
   $GSTACK_BIN/gstack-distill-free-text 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-distill-free-text skipped"
   ```
...
```bash
$GSTACK_BIN/gstack-distill-free-text --background 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-distill-free-text skipped"
```
