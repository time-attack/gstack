...
```bash
$GSTACK_BIN/gstack-gbrain-detect 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-gbrain-detect skipped"
```
...
```bash
$GSTACK_BIN/gstack-gbrain-install 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-gbrain-install skipped"
```
...
$GSTACK_BIN/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-gbrain-supabase-provision skipped"
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  $GSTACK_BIN/gstack-gbrain-mcp-verify "$MCP_URL" 2>/dev/null)
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  $GSTACK_BIN/gstack-gbrain-repo-policy set "$REMOTE" "$TIER" 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-gbrain-repo-policy skipped"
  ```
...
$GSTACK_BIN/gstack-artifacts-init --url-form-supported "$URL_FORM" 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-artifacts-init skipped"
$GSTACK_BIN/gstack-config set artifacts_sync_mode artifacts-only 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-config skipped"
...
$GSTACK_BIN/gstack-gbrain-source-wireup --strict \
  ${GBRAIN_URL:+--database-url "$GBRAIN_URL"} 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-gbrain-source-wireup skipped"
```
...
```bash
$GSTACK_BIN/gstack-memory-ingest --probe 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-memory-ingest skipped"
```
...
After answer:
```bash
$GSTACK_BIN/gstack-config set transcript_ingest_mode <choice> 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-config skipped"
$GSTACK_BIN/gstack-gbrain-sync --full --no-brain-sync 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-gbrain-sync skipped"
```
...
```bash
$GSTACK_BIN/gstack-config set brain_trust_policy@$_HASH personal 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-config skipped"
...
```bash
$GSTACK_BIN/gstack-config set brain_trust_policy@$_HASH <personal|shared> 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-config skipped"
```
...
if [ "$_CURRENT_SYNC" = "off" ]; then
  $GSTACK_BIN/gstack-config set artifacts_sync_mode full 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-config skipped"
