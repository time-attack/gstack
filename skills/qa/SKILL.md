...
- Print the required header with `Depth: readiness (trivial check)`, `Active modules: none (trivial-change fast path)`, and `Skipped modules: all (trivial-change fast path)`. Skip every per-invocation reference from dispatch step 4 — the probe already answered everything a module would ask.
