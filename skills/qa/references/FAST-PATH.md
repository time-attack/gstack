...
```text
Target: <the URL, device, repository, or artifact this run checked>
Mutation: <report-only, or the exact authorized mutation boundary>
```

Every other label in the full block is dropped, not filled with a guess. This path reads no specialist module and rates no depth, so a printed Mode, Depth, Active modules, Skipped modules, or Web context line would carry no information a reader can act on. Say in one sentence that the trivial-check fast path fired and that no module was read. That is the whole account of what was skipped.
