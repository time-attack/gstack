...
Review the `git diff <remote>/<base>` output (where `<remote>` and `<base>` are the remote and base branch detected earlier by `references/BASE-DETECTION.md`; never assume the remote is named `origin`, and with no remote `<remote>/<base>` means the local `<base>` ref) for the issues listed below. Be specific — cite `file:line` and suggest fixes. Skip anything that's fine. Only flag real problems.
