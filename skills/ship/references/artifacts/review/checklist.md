...
Review the `git diff origin/<base>` output (where `<base>` is the base branch detected earlier) for the issues listed below. Be specific — cite `file:line` and suggest fixes. Skip anything that's fine. Only flag real problems.
