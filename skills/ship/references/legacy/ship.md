...
4. **Write the bump** (FRESH, or an approved rebump):
   ```bash
   $GSTACK_BIN/gstack-version-bump write --version "$NEW_VERSION" 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-version-bump skipped"
   ```
...
1. **`REDACT_PREPUSH: true` and `HOOK_INSTALLED: no` and `HOOKS_IN_GIT_DIR: yes`** —
   ...
   ```bash
   $GSTACK_BIN/gstack-redact install-prepush-hook 2>/dev/null || echo "GSTACK_RUNTIME_ABSENT: gstack-redact skipped"
   ```
...
through `gstack-version-bump` whenever that runtime is present; its `NO_RUNTIME`
fallback is the only hand-rolled VERSION/package.json write.
