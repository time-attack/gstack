...
```bash
eval "$(~/.gbrain/skills/gstack/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
_PLAN_DIR="${GSTACK_HOME:-$HOME/.gstack}/projects/${SLUG:-unknown}"
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
_PLAN_GLOB=$(~/.gbrain/skills/gstack/bin/gstack-config get plan_glob 2>/dev/null || echo "")
...
`~/.gbrain/skills/gstack/pr-prep/SKILL.md` (or the `pr-prep` skill wherever
