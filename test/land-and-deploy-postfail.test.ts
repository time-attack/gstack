...
describe("PR #1620 post-failure PR-state check in /land template", () => {
  ...
  test("Authoritative state query uses gh pr view --json", () => {
    const body = readTmpl();
    expect(body).toMatch(/gh pr view <NNN> --json state,mergeCommit,mergedAt,mergedBy/);
  });
  ...
  test("MERGED branch captures merge SHA via mergeCommit.oid", () => {
    const body = readTmpl();
    expect(body).toMatch(/gh pr view <NNN> --json mergeCommit -q \.mergeCommit\.oid/);
  });
  ...
  test("OPEN branch checks autoMergeRequest before treating as failure", () => {
    const body = readTmpl();
    expect(body).toMatch(/gh pr view <NNN> --json autoMergeRequest/);
